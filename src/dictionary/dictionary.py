...
from levenshtein import levenshtein, find_used
...
class Dictionary(Trie):
    ...
    def find_candidates(self, char_sets):
        ...
        return self._find_candidates_rec(-1, self.root, '', '', char_sets)

    def _find_candidates_rec(self, depth, cur_dict, checked_prefix, prefix, char_sets, last_dist=0, used_word=''):
        cand = set()
        if depth >= 0:
            dist, last_dist, self.enter_idx[depth] = levenshtein(prefix, char_sets, last_dist, self.v[depth], self.v[depth+1])
        else:
            dist = 0
        ...
        if self._end in cur_dict.keys(): # Is a word
            #if checked_prefix not in ['the','and','of','to','a','as','in','on','for','is','that','i','by','with','you','it','not','or','be','are','this','from','at','your','new','more','an','was','we','will','home']: # TODO REMOVE THIS CONDITION!
            used = find_used(self.enter_idx, depth, dist)
            cand.add(WordCandidate(checked_prefix, cur_dict[self._end][self._count], (len(checked_prefix)-dist)/float(len(char_sets)),sum([char_sets[u[1]][checked_prefix[u[0]]]['w']*char_sets[u[1]][checked_prefix[u[0]]]['t'] for u in used])))
